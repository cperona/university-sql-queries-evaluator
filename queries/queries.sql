...
select distinct nombre,apellido1,apellido2 from persona left join alumno_se_matricula_asignatura on persona.id=alumno_se_matricula_asignatura.id_alumno left join curso_escolar on alumno_se_matricula_asignatura.id_curso_escolar=curso_escolar.id where anyo_inicio='2018' or anyo_inicio='2019';
...
select grado.nombre as grau, asignatura.tipo as tipus, sum(asignatura.creditos) as total_creditos from grado join asignatura on grado.id = asignatura.id_grado group by grado.id, grado.nombre, asignatura.tipo;
